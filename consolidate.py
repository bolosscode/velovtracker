#!/usr/bin/env python3
"""
consolidate.py — Fetche le jour J-1 depuis l'API Grand Lyon et écrit
data/history/YYYY-MM-DD.json au format delta compact.
Lancé à 2h10 via cron-job.org → GitHub Actions.
"""
...
def normalize_row(row):
    try:
        tot = parse_stands(
            row.get('total_stands', '')
            or row.get('main_stands', '')
        )

        tav = tot.get('availabilities', {}) if isinstance(tot, dict) else {}

        bikes = safe_int(tav.get('bikes'))
        elec  = safe_int(tav.get('electricalBikes'))
        meca  = safe_int(tav.get('mechanicalBikes'))
        cap   = safe_int(tot.get('capacity') if isinstance(tot, dict) else 0)

        stands = (
            (cap - bikes)
            if cap else safe_int(tav.get('stands'))
        )
...
while True:
    ...
    try:
        ...
        print(
            f"  page {page_start} | {len(rows)} lignes | total={len(all_rows):,}",
            flush=True
        )
...
# ── Grouper par slot de 10 min ─────────────────────────────────────────────────
buffer = defaultdict(dict)  # {slot_ts: {number: row}}
